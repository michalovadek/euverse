...
# Months that exist in BOTH years - we can only compute a year-on-year
# difference for those. In May 2026 we have Jan-May for both 2025 and
# 2026; June-Dec are 2025-only and would silently drop out of diff(),
# making the chart misleading. Filter explicitly.
months_in_both <- last_two_years |>
  mutate(year_part = format(month_year, "%Y"),
         month = format(month_year, "%m")) |>
  distinct(month, year_part) |>
  count(month) |>
  filter(n == 2L) |>
  pull(month)
...
last_two_full <- expand.grid(month_year = unique(last_two_years$month_year),
  ...
  filter(month %in% months_in_both)
